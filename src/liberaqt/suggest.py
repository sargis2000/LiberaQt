...
def resolver_for(session, root_handle: str) -> Callable[[str], list]:
    ...
    Args:
        session: Live session to ask.
        root_handle: Object to search beneath, normally a window.

    Returns:
        A callable mapping a selector string to the handles it matches.
    """
...
def walk(node: dict | None, depth: int = 0) -> Iterator[tuple[int, dict]]:
    ...
    Args:
        node: The tree, or a subtree. ``None`` yields nothing.
        depth: Depth of ``node`` itself, used for the recursion.

    Yields:
        ``(depth, node)`` for every object, parents before children.
    """
    if not node:
        return
    yield depth, node
...
def suggest(node: dict, depth: int, resolve: Callable[[str], list[str]]) -> Suggestion:
    ...
    Candidates are tried best-first and the first that resolves to exactly this object wins.

    Args:
        node: The object to describe, from an ``object.tree`` dump.
        depth: Its depth in the tree, carried into the result for display.
        resolve: Maps a selector to the handles it matches, in engine order.

    Returns:
        The best candidate and how it fared: unique, positional, or ambiguous.
    """
...
def suggest_all(tree: dict | None, resolve: Callable[[str], list[str]],
    ...
    Args:
        tree: An ``object.tree`` dump. ``None`` yields nothing.
        resolve: Maps a selector to the handles it matches.
        include_internal: Keep Qt's own plumbing objects, such as
            ``qt_scrollarea_viewport``, which are normally noise.
        skip_root: Leave the root out, for the reason above.

    Returns:
        One suggestion per object, in tree order.
    """
    out = []
...
def format_table(suggestions: list[Suggestion], width: int = 46) -> str:
    ...
    Args:
        suggestions: What to show, in the order given.
        width: Column width for the selector, before the text and status columns.

    Returns:
        The table, or a line saying nothing was found.
    """
    if not suggestions:
