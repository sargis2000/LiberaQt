...
from .launcher import AgentEndpoint
...
if TYPE_CHECKING:
    from .launcher import AgentEndpoint, LaunchedProcess
...
class Application:
    ...
    def __init__(self, session: Session, process: LaunchedProcess | None = None):
        ...
        self._owner = None      # set by LiberaQt._register, so attach_child can reuse it
        self._closed = False
    ...
    @property
    def token(self) -> str:
        """The secret this application's agent requires, or empty when we merely attached."""
        return self._process.token if self._process else ""

    @property
    def child_agents(self) -> list[AgentEndpoint]:
        """Agents running in processes this application spawned.

        A Qt child inherits the injection environment, so it loads the agent and publishes a
        port of its own. That is what makes an out-of-process dialog drivable: Libero's core
        configurator is a separate ``coreconfig.exe``, invisible to this application's agent
        because it is a different process entirely, and reachable only through its own.

        Empty when this application was attached rather than launched -- there is no process
        tree of ours to look in.
        """
        return self._process.child_agents() if self._process else []

    def wait_for_child_agent(self, timeout: float = 30.0,
                             pid: int | None = None) -> AgentEndpoint:
        """Wait for a child process to publish an agent, and return it.

        Spawning one is not instant -- Libero's configurator takes several seconds to come up --
        so polling beats a fixed sleep.

        Args:
            timeout: Seconds to wait.
            pid: Wait for this process specifically, rather than for any child.

        Returns:
            The :class:`~liberaqt.launcher.AgentEndpoint` that appeared.

        Raises:
            TimeoutError: No child agent appeared in time.
        """
        def once():
            for endpoint in self.child_agents:
                if pid is None or endpoint.pid == pid:
                    return endpoint
            raise LiberaQtError("no child agent has published a port yet")

        return retry(once, timeout=timeout, description="a child agent to appear")

    def attach_child(self, endpoint: AgentEndpoint | None = None, *, pid: int | None = None,
                     object_map: str | None = None, timeout: float = 30.0) -> Application:
        """Connect to an agent in a process this application spawned.

        The token is shared by inheritance, so nothing extra is needed to authenticate.

        Args:
            endpoint: A specific endpoint from :attr:`child_agents`. Omit to take the first
                child that appears, waiting for one if necessary.
            pid: Wait for this process specifically, when ``endpoint`` is omitted.
            object_map: Path to a YAML object map for the child.
            timeout: Seconds to wait for a child agent when one is not given.

        Returns:
            A connected :class:`Application` for the child process.

        Raises:
            LiberaQtError: This application was attached rather than launched, so it has no
                process tree to search.
            TimeoutError: No child agent appeared in time.
        """
        if self._owner is None:
            raise LiberaQtError(
                "attach_child needs the LiberaQt that launched this application",
                hint="Use LiberaQt.connect(port, token=...) directly when attaching by hand.",
            )
        if endpoint is None:
            endpoint = self.wait_for_child_agent(timeout=timeout, pid=pid)
        return self._owner.connect(endpoint.port, token=self.token, object_map=object_map)
    ...
    def capabilities(self) -> dict:
        """What the connected agent reports it can do: commands, Quick support, ABI.

        See :attr:`liberaqt.session.Session.capabilities` for why this is separate from the
        protocol version.
        """
        return self._session.capabilities

    def supports(self, cmd: str) -> bool:
        """Whether the agent implements a command.

        Args:
            cmd: A :class:`~liberaqt.protocol.Cmd` value.

        Returns:
            True if available, or if the agent is too old to say.
        """
        return self._session.supports(cmd)

    @property
    def agent_version(self) -> str:
        """Version of the agent binary this application loaded."""
        return self._session.agent_version
