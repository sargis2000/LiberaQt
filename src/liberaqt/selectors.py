...
def coerce(selector: Any = None, /, **kwargs: Any) -> Selector:
    ...
    Args:
        selector: A selector string, a dict of selector fields, or a parsed ``Selector``.
            Positional-only, so ``coerce("QPushButton")`` reads naturally.
        **kwargs: Selector fields as keywords, e.g. ``type="QPushButton"``. Mutually exclusive
            with ``selector``.
    ...
    Raises:
        InvalidSelectorError: Both forms were given at once, or the value is not something a
            selector can be made from.
    """
...
def join(parent: Selector, child: Selector) -> Selector:
    """Concatenate two selectors, as descendant steps.

    Args:
        parent: The outer selector.
        child: Matched beneath ``parent``, at any depth.

    Returns:
        A new selector; neither argument is modified.
    """
