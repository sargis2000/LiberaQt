...
class Session:
    ...
    def capabilities(self) -> dict:
        """What the connected agent reports it can do.

        Separate from the protocol version on purpose. ``protocol`` is the strict gate on the
        wire format and a mismatch refuses the connection; this describes features *within* that
        format, which move independently -- an agent registers fewer commands than the client
        knows names for, and Quick support depends on whether Qt Quick was found when it was
        built.
    ...
    def commands(self) -> frozenset[str]:
        """Command names this agent answers.

        Returns:
            The registered commands, or an empty set if the agent predates capability reporting
            -- in which case :meth:`supports` cannot tell and answers True.
        """
        return frozenset(self.capabilities.get("commands") or ())

    def supports(self, cmd: str) -> bool:
        """Whether the agent implements a command.

        Lets a caller check before acting rather than discovering it through an
        :class:`~liberaqt.errors.UnsupportedOperationError`. An older agent that reports no
        capabilities answers True, because "unknown" must not read as "missing" and the call
        itself still fails cleanly.

        Args:
            cmd: A :class:`~liberaqt.protocol.Cmd` value, e.g. ``"quick.evaluate"``.

        Returns:
            True if the command is available, or if the agent does not say.

        Example:
            Skip work the agent cannot do::

                if session.supports(Cmd.QUICK_EVALUATE):
                    session.call(Cmd.QUICK_EVALUATE, {...})
        """
        known = self.commands
        return True if not known else cmd in known

    @property
    def agent_version(self) -> str:
        """Version of the agent binary, as it reported itself.

        Not compared against the client version: they are released together but installed
        separately, and the compatibility question the client actually enforces is the protocol
        version. This is here for diagnostics.
        """
        return str(self.transport.hello.get("agent") or "")
