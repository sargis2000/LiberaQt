...
public:
    ...
    // Every command this build actually answers, sorted. Sent in the hello banner so a client can
    // ask what is supported instead of discovering it by calling and being refused: the Python
    // Cmd table lists more names than any one agent registers, and an agent built without Qt
    // Quick registers fewer still.
