...
QVariantMap WidgetBackend::describe(QObject *object, ObjectRegistry &registry)
{
    ...
    // window_type was only ever sent by window.list, but Window.kind reads object.info -- so it
    // fell back to its default and reported every window as a widget, including a QQuickView.
    if (object->isWindowType() || (qobject_cast<QWidget *>(object)
                                   && qobject_cast<QWidget *>(object)->isWindow())) {
        info.insert(QStringLiteral("window_type"),
                    qobject_cast<QWidget *>(object) ? QStringLiteral("widget")
...
QString WidgetBackend::actionabilityProblem(QObject *object, bool nativeInput)
{
    ...
    // ...unless the target is inside the active popup. Qt routes input to a popup ahead of
    // everything else, which is exactly how a combo box list or a completer dropdown stays
    // usable while a modal dialog is up. Found on Libero: the Import Files dialog is modal and
    // its completer dropdown is a separate top-level popup, so the test below called a dropdown
    // entry unreachable when a user can plainly click it.
    QWidget *activePopup = QApplication::activePopupWidget();
    const bool insidePopup = activePopup
                             && (activePopup == widget || activePopup == widget->window()
                                 || activePopup->isAncestorOf(widget));

    if (QWidget *modal = insidePopup ? nullptr : QApplication::activeModalWidget()) {
...
// What counts as "visual" for a tree dump: something a user could actually see.
bool isVisualObject(QObject *object)
{
    if (qobject_cast<QWidget *>(object) || qobject_cast<QWindow *>(object))
        return true;
#ifdef LIBERAQT_HAVE_QUICK
    if (QuickBackend::isItem(object))
        return true;
#endif
    return false;
}
                                    ...
                                    bool visualOnly)
{
    ...
    // The same enumeration object.find uses, which is the whole fix for QML: this walked raw
    // children() and so never stepped from a QQuickWindow into its contentItem, while find()
    // -- going through visualChildren() -- always did. That is why `liberaqt inspect` reported
    // "no objects found" for a Quick window that object.find could see 40 objects inside.
    const auto kids = SelectorEngine::visualChildren(root);
    for (QObject *child : kids) {
        // dumpTree does not go through matchesStep, so it repeats the exclusion itself.
        if (child->property(LIBERAQT_INTERNAL_PROPERTY).toBool())
            continue;
        // A QQuickItem is neither a QWidget nor a QWindow, so the old test dropped every one of
        // them even once the walk reached them.
        if (visualOnly && !isVisualObject(child))
            continue;
...
QVariantMap accessibleNode(QAccessibleInterface *iface, int depth)
{
    QVariantMap node;
    if (!iface || !iface->isValid())
        return node;

    node.insert(QStringLiteral("name"), iface->text(QAccessible::Name));
    node.insert(QStringLiteral("description"), iface->text(QAccessible::Description));
    node.insert(QStringLiteral("value"), iface->text(QAccessible::Value));
    node.insert(QStringLiteral("role"), static_cast<int>(iface->role()));
    node.insert(QStringLiteral("child_count"), iface->childCount());

    // Screen coordinates, which is what QAccessible deals in -- the point of the whole exercise
    // when the elements have no QObject to ask for a geometry.
    const QRect r = iface->rect();
    node.insert(QStringLiteral("rect"), QVariantList{r.x(), r.y(), r.width(), r.height()});

    if (QAccessibleActionInterface *actions = iface->actionInterface()) {
        QVariantList names;
        for (const QString &name : actions->actionNames())
            names.append(name);
        node.insert(QStringLiteral("actions"), names);
    }
    ...
    for (int i = 0; i < iface->childCount(); ++i) {
        if (QAccessibleInterface *kid = iface->child(i))
            children.append(accessibleNode(kid, depth > 0 ? depth - 1 : -1));
    }
    node.insert(QStringLiteral("children"), children);
    return node;
}
...
QVariantMap WidgetBackend::accessibleTree(QObject *object, int depth)
{
    QVariantMap out;
    // Reported so a caller can tell "the widget exposes nothing" from "accessibility is off in
    // this process", which look identical from the outside and mean quite different things.
    out.insert(QStringLiteral("accessibility_active"), QAccessible::isActive());

    QAccessibleInterface *iface = QAccessible::queryAccessibleInterface(object);
    if (!iface || !iface->isValid()) {
        out.insert(QStringLiteral("supported"), false);
        return out;
    }
    out.insert(QStringLiteral("supported"), true);
    out.insert(QStringLiteral("root"), accessibleNode(iface, depth));
    return out;
}

bool WidgetBackend::highlightTarget(QObject *object, const QString &handle,
                                    QWidget **topLevel, QRect *rect)
{
    ...
    // A composite handle names a cell, so highlight the cell. indexFromHandle answers invalid
    // for a plain view handle, which falls through to the widget rect below.
    if (auto *view = qobject_cast<QAbstractItemView *>(widget)) {
        ...
        if (index.isValid()) {
            ...
            if (cell.isValid()) {
                // visualRect() is in viewport coordinates; the overlay lives on the top level.
                *topLevel = top;
                *rect = QRect(view->viewport()->mapTo(top, cell.topLeft()), cell.size());
                return true;
            }
        }
    }
    ...
    *topLevel = top;
    *rect = QRect(widget->mapTo(top, QPoint(0, 0)), widget->size());
    return true;
}
