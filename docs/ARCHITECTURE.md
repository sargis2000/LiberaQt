...
A driver process in Python talks to an agent loaded inside the application under test, and the
thing it addresses is the application's live `QObject` tree.
...
slots. The only way to reach them is from inside the process, which is what the agent is for. It gives us:
...
QT_PLUGIN_PATH=<every installed agent's plugins dir>
LIBERAQT_PORT=0                 # 0 => bind an ephemeral port
LIBERAQT_PORT_DIR=<temp dir>    # each agent writes <pid>.port here
LIBERAQT_TOKEN=<random>
```

Each agent build advertises a key naming its Qt version, pointer size and compiler
(`liberaqt_5_15_64_msvc`), because Qt binds a plugin key to exactly one library: while every
build claimed only `liberaqt`, the first directory on the path owned the key and every other ABI
in the process tree was locked out in silence.
...
Two layers, which is what makes a suite maintainable at scale:
...
License: **Apache-2.0** (permissive, patent grant, safe for commercial adoption — the agent is
loaded into proprietary applications, so a copyleft licence would raise a combined-work question
that stops legal review even when the answer is no).
