...
| `type` | Type character by character; `<Ctrl+A>` chords may ride in the text (`Keyboard.type` too) | yes | yes | Native aims at the *window* and Qt gives the keys to its focus widget; synthetic aims at the named widget directly. Keyboard shortcuts fire in both (measured). A target that cannot take focus is **refused**, rather than typing into whatever has focus instead and reporting success; aiming at a window is still fine, which is how `Window.keyboard.*` works. |
...
| `fill` / `clear` | Set or clear text in one shot | prop | prop | Deliberately not input, in any mode: writes the `text` property so change signals fire once, for state that is not itself under test. Refuses a read-only **or disabled** field — succeeding where a user could not type is a false pass. Does **not** require visibility: filling a field on a form page that is not currently shown is exactly what a state-setup helper is for. |
