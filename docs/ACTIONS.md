...
| `context_menu` | Right-click, then activate an entry in the menu that appears | yes | no | Squish's `openItemContextMenu` + `activateItem`. Works on item rows and cells (`row(...).context_menu(...)`), and `>` walks a submenu. Native only, by nature: the menu is built inside `contextMenuEvent`, so there is no action to trigger without genuinely opening it. A wrong entry lists what the menu really offers, which is also how to discover one. |
