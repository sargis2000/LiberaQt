...
* Recorder — mostly built, not wired. `spy.py`, `codegen.py`, `liberaqt record` and the agent's
  `Recorder` event filter all exist; `liberaqt record` still fails at once because nothing
  registers the commands. Remaining, in order:
  1. register `record.start` / `record.stop` in `dispatcher.cpp` and emit the `record.action`
     event from the filter;
  2. capture text entry — `recorder.cpp`'s `FocusOut` branch is empty, so today a recording
     contains clicks and nothing typed;
  3. verify each selector is unique by re-running it through the engine before emitting, or the
     generated test will not resolve;
  4. drive a real application by hand and read what comes out. Nothing else will tell you
     whether the inferred actions are the ones a person would have written.
...
* Documentation site, and a migration guide for suites written against another tool.
...
* Recorder produces runnable tests against real applications (there are no sample apps).
