...
Modern UI automation for Qt desktop applications. Python only, open source,
`pip install`-able.

Scoped deliberately narrowly: **Python** as the only test language,
**Qt 5.15, 6.5 and 6.7** as the exercised versions, **Windows and Linux** as the platforms, and
...
liberaqt doctor                      # client, Python, agent cache
```

Then get an agent matching the application you want to drive:

```bash
liberaqt agents kits                 # Qt kits on this machine
liberaqt agents build --qt 6.7       # build and install one
liberaqt doctor ./build/myapp        # verify the pieces line up
```

The agent is a Qt plugin, so it must match your Qt minor version, compiler and architecture.
`doctor` tells you when it does not, and names what to build.

Working on LiberaQT itself instead? `pip install -e ".[dev]"` from a checkout. The
[install guide](docs/getting-started/install.md) covers every form, including offline installs.
